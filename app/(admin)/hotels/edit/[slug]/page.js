...
import { getParticularHotelUrl, updateHotelUrl, getAllCityUrl, getAllZoneUrl } from '@/app/routes/serviceRoutes';
...
export default function EditHotelPage() {
    ...
    const hotelId = rawId ? urlDecode(rawId) : null;
    ...
    useEffect(() => {
        ...
        // Fetch Hotel Details
        axios.get(`${getParticularHotelUrl}?id=${rawId}`, {
            headers: { 'Authorization': `Bearer ${token}` }
        }).then((res) => {
            if (res.data && res.data.status && res.data.hotel) {
                const hotel = res.data.hotel;
                setFormData({
                    name: hotel.name || '',
                    star_rating: hotel.star_rating || 3,
                    hotel_type: hotel.hotel_type || 'Resort',
                    ...
                    tags: hotel.tags ? (typeof hotel.tags === 'string' ? hotel.tags.split(',').map(s => s.trim()) : hotel.tags) : []
                });

                setSelectedAmenities(Array.isArray(hotel.amenities) ? hotel.amenities : []);
                setRoomTypes(Array.isArray(hotel.room_types) ? hotel.room_types : []);
                setSelectedPackageIds((hotel.package_ids || []).map(Number));

                if (hotel.main_image) {
                    setExistingMainImage(hotel.main_image);
                }
                if (Array.isArray(hotel.images)) {
                    setExistingGallery(hotel.images);
                }
            } else {
                showMessage(res.data?.msg || 'Hotel not found', 'error');
                router.push('/hotels');
            }
        }).catch((err) => {
            console.error('Error fetching hotel details:', err);
            showMessage('Error fetching hotel details', 'error');
    ...
    const handleRemoveExistingGalleryImage = (imagePath) => {
        setExistingGallery(existingGallery.filter(img => img !== imagePath));
        setDeletedGallery(prev => [...prev, imagePath]);
    };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            uploadData.append('hotelId', rawId);
            ...
            if (deletedGallery.length > 0) {
                uploadData.append('delImages', JSON.stringify(deletedGallery));
            }

            if (newMainImage) {
                uploadData.append('main_image', newMainImage);
            }
            ...
            const response = await axios.put(updateHotelUrl, uploadData, {
                headers: {
            ...
            if (response.data && response.data.status) {
                ...
                showMessage(response.data?.msg || 'Failed to update hotel', 'error');
            }
        } catch (error) {
            console.error('Error updating hotel:', error);
            showMessage(error?.response?.data?.msg || error.message || 'Error updating hotel', 'error');
        } finally {
            setSubmitting(false);
        }
    };
    ...
    const currentMainImageSrc = newMainImagePreview || (existingMainImage
        ? (existingMainImage.startsWith('http') || existingMainImage.startsWith('/')
            ? existingMainImage
            : `${process.env.NEXT_PUBLIC_SERVER_URL}${existingMainImage}`)
        : null);
    ...
    return (
        <div className="container-xxl flex-grow-1 container-p-y">
            ...
            <div className="d-flex align-items-center mb-6 gap-3">
                ...
                <div>
                    <h4 className="fw-semibold mb-0">Edit Hotel: {formData.name}</h4>
                    <p className="text-muted small mb-0">
                        Update hotel information, amenities, room configurations, and linked tour packages
                    </p>
                </div>
            </div>
                            ...
                            <div className="card-body py-4 text-center">
                                {currentMainImageSrc ? (
                                    <div className="position-relative rounded-3 overflow-hidden border mb-3">
                                        ...
                                        <button
                                            type="button"
                                            onClick={() => { setNewMainImage(null); setNewMainImagePreview(null); setExistingMainImage(null); }}
                                ...
                                {currentMainImageSrc && (
                                    ...
                                    >
                                        <i className="ri-image-edit-line me-1"></i> Change Photo
                                    </button>
                                )}
                            </div>
                        </div>
                            ...
                            <div className="card-body py-4">
                                ...
                                {existingGallery.length === 0 && newGalleryPreviews.length === 0 ? (
                                    ...
                                    <div className="row g-2">
                                        ...
                                        {existingGallery.map((img, eIdx) => {
                                            const src = img.startsWith('http') || img.startsWith('/')
                                                ? img
                                                : `${process.env.NEXT_PUBLIC_SERVER_URL}${img}`;
                                            return (
                                                <div key={`existing-${eIdx}`} className="col-6 position-relative">
                                        ...
                                        {newGalleryPreviews.map((preview, nIdx) => (
                                            <div key={`new-${nIdx}`} className="col-6 position-relative">
                                                <div className="rounded-2 overflow-hidden border border-success" style={{ height: '90px' }}>
                                                    <img
                                                        src={preview}
                                                        alt="New gallery"
                                                        className="w-100 h-100 object-fit-cover"
                                                ...
                                                <button
                                                    type="button"
                                                    onClick={() => handleRemoveNewGalleryImage(nIdx)}
                                                    className="btn btn-danger btn-xs position-absolute top-0 end-0 m-1 rounded-circle p-1"
                                                    style={{ width: '22px', height: '22px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
                                                >
                                                    <i className="ri-close-line" style={{ fontSize: '12px' }}></i>
