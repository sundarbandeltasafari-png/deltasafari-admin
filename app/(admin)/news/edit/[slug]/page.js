...
import { getAllCategoryUrl, getAllZoneUrl, getParticularNewsUrl, updateNewsUrl } from '@/app/routes/serviceRoutes';
...
export default function page() {
  ...
  async function getParticularNews() {
    try {
      const response = await axios.get(`${getParticularNewsUrl}?id=${postId}`, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
  ...
  const handleZoneSelection = (zone) => {
    setSelectedZoneParent({ id: zone.id, name: zone.name });
      ...
      <div className="row justify-content-center">
        <div className='col-lg-11 card mb-3'>
          <div className=' card-header d-flex justify-content-between p-3 pb-4'>
            <div>
              <h3>Edit News</h3>
              <p className='mb-0'>Manage your news beautifully</p>
            </div>
          </div>
        </div>
        {loading ?
