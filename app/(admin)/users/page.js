...
function page() {
    ...
    function handleSearch() {
        setLoading(true)
        axiosPost(getUserSearchUrl, { searchData: searchData, status: usersFilterStatus }, token).then((res) => {
            if (res.status) {
                setusers(res.users)
            } else {
    ...
    useEffect(() => {
        axiosGet(getAllUsersUrl, token).then((res) => {
            if (res.status) {
                setLoading(false);
                setusers(res.users);
            }
        })
        axiosGet(getUserStatusUrl, token).then((res) => {
            if (res.status) {
            ...
            <div className="container-xxl flex-grow-1 container-p-y">
                ...
                <div className="card">
                    <div className="card-header border-bottom">
                        <FilterUser type={''} setUserStatus={setUsersFilterStatus} />
                    </div>
                                                ...
                                                <thead>
                                                    <tr>
                                                        ...
                                                        <th data-dt-column="4" rowSpan="1" colSpan="1" className="dt-orderable-asc dt-orderable-desc" aria-label="Role: Activate to sort" tabIndex="0">
                                                            <span className="dt-column-title" role="button">Joining</span>
                                                            <span className="dt-column-order"></span>
                                                        </th>
                                                                ...
                                                                <div className="d-flex justify-content-start align-items-center user-name">
                                                                    <div className="avatar-wrapper">
                                                                        <div className="avatar avatar-sm me-4">
                                                                            <img src={user?.profile_picture ? process.env.NEXT_PUBLIC_SERVER_URL + user?.profile_picture : (user?.gender == 1 ? "/assets/img/avatars/1.png" : user?.gender == 2 ? "/assets/img/avatars/2.png" : "/assets/img/avatars/7.png")} alt="Avatar" className="rounded-circle" />
                                                                        </div>
                                                                    </div>
